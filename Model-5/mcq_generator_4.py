#mcq generator with option to remove unwanted chunks of pdf
import locale
import weaviate
import random
import gradio as gr
...
def generate_mcqs_interface_pdf(pdf_path, num_questions, selected_chunks):
    if num_questions <= 0:
...
def display_chunks_for_selection(pdf_path):
    docs, _ = process_pdf(pdf_path)
...
with gr.Blocks() as iface:
    ...
    with gr.Tabs():
        with gr.TabItem("Upload PDF"):
            ...
            def update_chunks(pdf_path):
                chunk_labels = display_chunks_for_selection(pdf_path)
            ...
            generate_button.click(
                ...
                inputs=[pdf_input, num_questions_pdf, chunk_selection],
                outputs=[pdf_output]
            )
