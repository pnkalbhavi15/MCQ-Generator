#mcq generator for any no of pdfs based on user input, done in streamlit - updated version of mcq_generator_6.py
import re
import locale
import weaviate
import random
...
if tab == "Upload PDFs":
    ...
    if all(pdf_files):
        ...
        if st.button("Generate MCQs"):
            if pdf_files:
                mcqs = generate_mcqs_from_multiple_pdfs([file for file in pdf_files], page_ranges_list, num_questions, st.session_state.exclude_indices)
    ...
    if st.button("Generate MCQs"):
        if text:
            mcqs = generate_mcqs_interface_text(text, num_questions)
            st.text_area("Generated MCQs", mcqs, height=300)
